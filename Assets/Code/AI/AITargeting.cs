...
public class AITargeting 
{
	...
	{
		Vector3 relativeVelocity = target.MyNavAgent.velocity - _parentCharacter.MyNavAgent.velocity;

		float colliderHeight = target.GetComponent<CapsuleCollider>().height;
		return target.transform.position + Vector3.up * colliderHeight * 0.5f + relativeVelocity / 3;
	}
